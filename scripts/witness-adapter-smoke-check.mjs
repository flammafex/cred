...
if (!recordPath || !presentationPath || !verifyPath) {
  throw new Error('usage: witness-adapter-smoke-check.mjs <record> <presentation> <verify>');
}
...
if (artifact?.artifact_type !== 'witness.signed_attestation') {
...
if (artifact.artifact_hash !== record.artifact_hash) {
  throw new Error('presentation artifact_hash did not match stored Witness record hash');
}
if (artifact.disclosure !== 'reference') {
  throw new Error(`unexpected disclosure: ${artifact.disclosure}`);
}
if (artifact.artifact !== undefined) {
  throw new Error('record-backed Witness presentation embedded raw artifact material');
}
...
console.log('Cred Witness adapter smoke passed.');
